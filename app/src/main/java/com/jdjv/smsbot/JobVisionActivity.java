...
public class JobVisionActivity extends Activity {
    ...
    private void doLogin() {
        ...
        String js =
            ...
            "      if(best) break;" +
            "    }" +
            "    if(best){ tapElement(best); return true; }" +
            "    return false;" +
            "  }" +
            "  function step1() {" +
            ...
            "      if(passEl){ clearInterval(interval); step2(passEl); }" +
            "      else if(tries === 6){ clickByText(['ادامه','Continue','Next','بعدی']); }" +
    ...
    private void extractResume(final String url) {
        ...
        String js =
            ...
            "  if (!name) { var titleParts = document.title.split('|'); name = titleParts[0].trim(); }" +
            "  var g = 'unknown';" +
